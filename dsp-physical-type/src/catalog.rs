...
//!
//! Phases 4.1–4.3 produced sealed `.dspseg` segments — typed columns, per-segment
...
//! next slice, persist in libSQL) so it prunes *before* touching any `.dspseg` byte.
//!
...
/// segment, enough to prune a query against it without opening its `.dspseg`.
///
...
pub struct SegmentDescriptor {
	...
	pub id: u64,
	/// Where the sealed `.dspseg` frame lives — the locator a pruned query opens.
	pub path: String,
	...
	/// The realized on-disk size of the sealed `.dspseg` frame, in bytes — the
...
impl SegmentDescriptor {
	...
	/// opening the `.dspseg`. Mirrors [`Segment::overlaps_time`](crate::Segment::overlaps_time).
	#[must_use]
...
/// exist for: a bounded range query consults the index — never the `.dspseg`
...
impl SegmentIndex {
	...
	/// on its min/max stats alone, without opening its `.dspseg`. Insertion order is
	/// preserved.
	#[must_use]
...
mod tests {
	...
	fn descriptor_copies_segment_stats_verbatim() {
		...
		let d = SegmentDescriptor::of_segment(7, "segments/7.dspseg", len, &seg);
		assert_eq!(d.id, 7);
		assert_eq!(d.path, "segments/7.dspseg");
	...
	fn descriptor_pruning_mirrors_the_segment() {
		let (seg, len) = sealed(100);
		let d = SegmentDescriptor::of_segment(0, "s.dspseg", len, &seg);
	...
	fn descriptor_from_paged_segment() {
		...
		let len = seg.write_to().len() as u64;
		let d = SegmentDescriptor::of_paged_segment(3, "p.dspseg", len, &seg);
		assert_eq!(d.format_version, seg.version);
	...
	fn empty_paged_descriptor_has_no_encoding() {
		...
		let d = SegmentDescriptor::of_paged_segment(0, "empty.dspseg", seg.write_to().len() as u64, &seg);
	...
	fn nullable_descriptor_tracks_null_count() {
		...
		let d = SegmentDescriptor::of_segment(1, "n.dspseg", seg.write_to().len() as u64, &seg);
		...
		let ad = SegmentDescriptor::of_segment(2, "an.dspseg", all_null.write_to().len() as u64, &all_null);
	...
	fn index_prunes_to_overlapping_segments() {
		...
		for (i, base) in [0_i64, 100, 200].into_iter().enumerate() {
			...
			index.push(SegmentDescriptor::of_segment(i as u64, format!("s{i}.dspseg"), len, &seg));
		}
	...
	fn index_value_pruning_selects_disjoint_spans() {
		...
		for (i, base) in [0_i64, 100, 200].into_iter().enumerate() {
			...
			let seg = Segment::build(&ts, &vs, TimeUnit::Seconds, &BigDecimal::from(0)).expect("builds");
			index.push(SegmentDescriptor::of_segment(i as u64, format!("v{i}.dspseg"), seg.write_to().len() as u64, &seg));
		}
		let ids = |ds: Vec<&SegmentDescriptor>| ds.iter().map(|d| d.id).collect::<Vec<_>>();
	...
	fn index_quality_pruning_drops_all_null_segments() {
		...
		index.push(SegmentDescriptor::of_segment(0, "d.dspseg", dlen, &dense));
		...
		index.push(SegmentDescriptor::of_segment(1, "a.dspseg", all_null.write_to().len() as u64, &all_null));
		let ids = |ds: Vec<&SegmentDescriptor>| ds.iter().map(|d| d.id).collect::<Vec<_>>();
	...
	fn index_serde_round_trips() {
		...
		index.push(SegmentDescriptor::of_segment(0, "s0.dspseg", len, &seg));
