...
/// - **v6** — each page's timestamp block gains the per-block adaptive (dynamic)
///   bit-pack codec option; see [`SEGMENT_FORMAT_VERSION`](crate::SEGMENT_FORMAT_VERSION) v5.
pub const PAGED_SEGMENT_FORMAT_VERSION: u16 = 6;
...
impl PagedSegment {
	...
	/// page-level stat *tables* are frame overhead accounted by the `.weftseg` layout,
	/// not here.
	#[must_use]
	pub fn total_bytes(&self) -> usize {
	...
	/// Seal this paged segment to its on-disk `.weftseg` byte frame (format version
	/// 3): magic + header + per-page index + page blocks + trailing CRC-32. See
	/// [`crate::weftseg::write_paged_segment`].
	///
	/// Exact inverse of [`PagedSegment::read_from`].
	#[must_use]
	pub fn write_to(&self) -> Vec<u8> {
		crate::weftseg::write_paged_segment(self)
	}

	/// Seal to a `.weftseg` frame whose every page carries a **persisted checkpoint index**.
	/// See [`crate::weftseg::write_paged_segment_checkpointed`].
	///
	...
	pub fn write_to_checkpointed(&self, stride: usize) -> Vec<u8> {
		crate::weftseg::write_paged_segment_checkpointed(self, stride)
	}
	...
	pub fn write_to_with(&self, opts: &crate::weftseg::FrameOptions) -> Vec<u8> {
		crate::weftseg::write_paged_segment_with(self, opts)
	}
	...
	/// Read a paged segment back from a `.weftseg` byte frame, verifying its checksum.
	///
	/// Exact inverse of [`PagedSegment::write_to`]. See
	/// [`crate::weftseg::read_paged_segment`].
	///
	/// # Errors
	///
	...
	pub fn read_from(bytes: &[u8]) -> Result<Self, crate::weftseg::WeftSegError> {
		crate::weftseg::read_paged_segment(bytes)
	}
