...
//! `.weftseg` file. This module is the in-memory shape of that segment: the typed
...
//!   Weft-Bench `StorageEstimate`** — the advisory estimate the bench already
...
/// on. A sealed `.weftseg` carries this so future Storage v2 code can refuse or
...
impl Segment {
	...
	/// `.weftseg` frame writes under the codec it actually selects (see
	...
	/// Weft-Bench `StorageEstimate::total_bytes_per_point` for the same data, by
	...
	/// Seal this segment to its on-disk `.weftseg` byte frame (magic + header +
	/// column blocks + trailing CRC-32). See [`crate::weftseg::write_segment`].
	///
	/// Exact inverse of [`Segment::read_from`].
	#[must_use]
	pub fn write_to(&self) -> Vec<u8> {
		crate::weftseg::write_segment(self)
	}

	/// Seal to a `.weftseg` frame carrying a **persisted checkpoint index** over the
	/// timestamp column, so a point lookup resumes from the nearest checkpoint instead of
	/// decoding the whole column. See [`crate::weftseg::write_segment_checkpointed`].
	///
	...
	pub fn write_to_checkpointed(&self, stride: usize) -> Vec<u8> {
		crate::weftseg::write_segment_checkpointed(self, stride)
	}

	/// Seal to a `.weftseg` frame under explicit [`FrameOptions`](crate::weftseg::FrameOptions)
	/// — the general form of [`write_to`](Self::write_to) (which is this with
	/// [`FrameOptions::DEFAULT`](crate::weftseg::FrameOptions::DEFAULT)) and
	...
	#[must_use]
	pub fn write_to_with(&self, opts: &crate::weftseg::FrameOptions) -> Vec<u8> {
		crate::weftseg::write_segment_with(self, opts)
	}
	...
	/// Read a segment back from a `.weftseg` byte frame, verifying its checksum.
	///
	/// Exact inverse of [`Segment::write_to`]. See [`crate::weftseg::read_segment`].
	///
	/// # Errors
	///
	/// Propagates [`crate::weftseg::WeftSegError`] for a corrupt, truncated, or
	/// unrecognised frame (checksum mismatch, bad magic, unsupported version, …).
	pub fn read_from(bytes: &[u8]) -> Result<Self, crate::weftseg::WeftSegError> {
		crate::weftseg::read_segment(bytes)
	}
