...
///
/// WeftDB holds timestamps as integer epochs internally; the unit is declared per
...
/// on-disk realization (a checkpoint block in the `.weftseg` timestamp column behind a
...
/// value rides in the `.weftseg` block (a uvarint), so it can change without breaking
/// old frames.
...
///
/// Roadmap Phase 6.1 "dynamic bit packing" — realized on disk (the `.weftseg` timestamp
...
/// [`ByteWriter::put_uvarint`](crate::weftseg) so the FOR prototype needs no `weftseg`
...
/// `.weftseg` writer (that is the adopt-or-drop slice). Comparable with the other
...
impl DeltaOfDeltaColumn {
	...
	///
	/// Now **realized on disk** (roadmap Phase 6.1): the `.weftseg` timestamp block
	...
	///
	/// Realized on disk (roadmap Phase 6.1): the `.weftseg` timestamp block carries a
	...
	/// [`best_estimated_bytes`](Self::best_estimated_bytes) or the `.weftseg` writer — the
	...
	/// never disagree on which won — the `.weftseg` writer routes through this name. Ties
