...
//! and Weft-Bench's headline cost metric is **storage bytes per point**. Both want
...
impl ColumnEncoding {
	...
	/// `.weftseg` value block writes for these values (the codec each variant uses in
	/// `weftseg::write_physical_value`), excluding the column header.
	///
	...
	/// size (it *undersells* WeftDB's realized bytes/point). This method is the accurate
	...
	///
	/// This is the single source of truth the `.weftseg` cascade writer routes through, so the
	...
	///
	/// This is what the on-disk cascade path ([`crate::weftseg::write_value_column_cascading`])
	...
	/// source of truth the `.weftseg` writer routes through.
	#[must_use]
	...
	/// ([`crate::weftseg::write_value_column_cascading`]) routes through. It is **not** the
...
///
/// Each maps to a `.weftseg` cascade-inner descriptor byte and reuses the crate's existing
...
/// [`ColumnEncoding::delta_cascade_plan`]; consumed by the `.weftseg` cascade writer so the
