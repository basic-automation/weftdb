...
/// The end-to-end claim: a real `.weftseg` point read on an irregular sorted frame, plain
...
fn bench_frame(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("weftseg_point_read_irregular_100k");
...
fn bench_paged_frame(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("weftseg_paged_point_read_irregular_100k");
...
fn report_codec_override_cost(_c: &mut Criterion) {
	// (name, timestamps) — the shapes WeftDB's timestamp columns actually take.
