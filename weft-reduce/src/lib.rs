...
//! WeftDB's aggregation semantics, shared across the surfaces that reduce a series into
//! grid-aligned buckets (the HTTP `downsample` endpoint, the `weft-bench` downsample
...
/// bucket selects a *different* sample — `p99` of three would be the middle one; WeftDB
...
mod tests {
	...
	fn sketch_handles_negative_and_zero_buckets() {
		...
		// The sketch uses WeftDB's nearest-rank convention, so even on a 3-sample bucket it
		// names the same sample the exact p99 does — the top one. (Under DDSketch's own
		// floor(q*(n-1)) rank it would have returned the middle sample; WeftDB deliberately
	...
	fn rebucket_returns_none_when_grids_do_not_nest() {
		// Weeks do not tile WeftDB's 30-day month, nor months a 365-day year; a finer target
