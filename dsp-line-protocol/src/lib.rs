//! # dsp-line-protocol
//!
//! `InfluxDB` Line Protocol (ILP) parsing for DSP ingest.
//!
...
//! own crate outside the core. It is shared by the benchmark harness (`dsp-bench`)
//! and the HTTP server (`dsp-server`) so both speak exactly the same dialect.
//!
