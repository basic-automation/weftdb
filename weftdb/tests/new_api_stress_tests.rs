...
async fn test_multiple_aspects_same_subject() {
	...
	for i in 0..num_aspects {
		let aspect = db.track_aspect(&subject.id(), &format!("aspect_{i}"), &splimes::Resolution::Milliseconds, None).await.expect("Failed to track aspect");
		aspects.push(aspect);
	}
...
async fn test_high_frequency_measurements() {
	...
	let aspect = db.track_aspect(&subject.id(), "high_freq_aspect", &splimes::Resolution::Microseconds, None).await.expect("Failed to track aspect");

	// Reduced dataset for faster testing
	let base_time = Utc.with_ymd_and_hms(2023, 1, 1, 12, 0, 0).unwrap();
	let num_measurements = 600; // 10 minutes of data

	println!("Capturing {num_measurements} high-frequency measurements...");
...
async fn test_concurrent_access() {
	// Follows the repo's `SKIP_SLOW_TESTS` convention (see `weftdb/tests/db_tests.rs`).
	// These two stress tests run for minutes at multi-GB RSS; under a parallel
	// `cargo test --workspace` on a memory-constrained machine the OOM killer takes the
	// whole test binary out, which surfaces as a SIGKILL rather than a failure. CI sets
	// SKIP_SLOW_TESTS=1; run without it to exercise them.
	if std::env::var("SKIP_SLOW_TESTS").is_ok() {
		eprintln!("skipping test_concurrent_access: SKIP_SLOW_TESTS is set");
		return;
	}
	...
	let aspect = db.track_aspect(&subject.id(), "concurrent_aspect", &splimes::Resolution::Milliseconds, None).await.expect("Failed to track aspect");
	...
	println!("Running {num_concurrent_tasks} concurrent tasks with {operations_per_task} operations each...");
...
async fn test_large_dataset_analysis() {
	...
	// SKIP_SLOW_TESTS=1; run without it to exercise them.
	if std::env::var("SKIP_SLOW_TESTS").is_ok() {
		eprintln!("skipping test_large_dataset_analysis: SKIP_SLOW_TESTS is set");
		return;
	}
	...
	let aspect = db.track_aspect(&subject.id(), "large_dataset_aspect", &splimes::Resolution::Seconds, None).await.expect("Failed to track aspect");
	...
	println!("Creating large dataset with {num_measurements} measurements...");
	let start = std::time::Instant::now();

	for i in 0..num_measurements {
		let timestamp = base_time + time_interval * i;
		let value = (f64::from(i) / 100.0).sin() * 50.0 + 50.0;
	...
	for (spline_type, name) in test_cases {
		...
		let analysis_duration = analysis_start.elapsed();
		println!("{name} analysis on large dataset completed in {analysis_duration:?}");
...
async fn test_memory_usage_stability() {
	...
	let aspect = db.track_aspect(&subject.id(), "memory_test_aspect", &splimes::Resolution::Milliseconds, None).await.expect("Failed to track aspect");
...
async fn test_edge_case_scenarios() {
	...
	let aspect = db.track_aspect(&subject.id(), "edge_case_aspect", &splimes::Resolution::Milliseconds, None).await.expect("Failed to track aspect");
