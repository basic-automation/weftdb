...
impl Relative {
	#[must_use]
	pub const fn new(vector: MeasurementVector, max_x: BigDecimal, max_y: BigDecimal) -> Self {
		Self { vector, max_x, max_y }
	}

	#[must_use]
	pub const fn vector(&self) -> &MeasurementVector {
		&self.vector
	}

	#[must_use]
	pub const fn max_x(&self) -> &BigDecimal {
		&self.max_x
	}

	#[must_use]
	pub const fn max_y(&self) -> &BigDecimal {
		&self.max_y
	}
