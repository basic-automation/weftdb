...
/// GPU accelerated interpolation context with buffer reuse for batching
#[derive(Debug)]
pub struct GpuInterpolator {
	device: Device,
	queue: Queue,
	pipelines_f64: HashMap<Method, ComputePipeline>,
	pipelines_f32: HashMap<Method, ComputePipeline>,
...
impl GpuInterpolator {
	...
	pub async fn new() -> Result<Self> {
		...
		println!("GPU: Reported max storage buffer binding size: {} bytes", max_storage_buffer_binding_size);
		// Cap at default limit (128 MiB) to ensure compatibility
		let max_storage_buffer_binding_size = max_storage_buffer_binding_size.min(128 * 1024 * 1024);
		println!("GPU: Using max storage buffer binding size: {} bytes", max_storage_buffer_binding_size);
		...
		let (device, queue) = adapter
			.request_device(
				&DeviceDescriptor {
					label: Some("Interpolation Device"),
					required_features: if supports_f64 { Features::SHADER_F64 } else { Features::empty() },
					required_limits: Limits::default(), // Use default limits
				},
				None,
			)
			.await
	...
	pub fn interpolate_f64(&mut self, input_times: &[f64], input_values: &[f64], target_times: &[f64], method: &Method) -> Result<Vec<f64>> {
		self.interpolate_impl(input_times, input_values, target_times, method, true)
	}

	pub fn interpolate_f32(&mut self, input_times: &[f32], input_values: &[f32], target_times: &[f32], method: &Method) -> Result<Vec<f32>> {
		...
		// Create input buffers (small, typically < 1000 points)
		...
		let pipelines = &mut self.pipelines_f32;
		...
		// Use max_storage_buffer_binding_size and max_compute_workgroups_per_dimension for batching
		let element_size = std::mem::size_of::<f32>();
		let workgroup_size = 256;
		let max_compute_workgroups = 65535; // From Limits::defaults()
		...
		// Process target_times in batches
		for target_batch in target_times.chunks(max_elements_per_batch) {
			let output_size = target_batch.len() * element_size;
...

			{
				...
				let num_workgroups = (target_batch.len() + workgroup_size - 1) / workgroup_size;
	...
	fn interpolate_impl(&mut self, input_times: &[f64], input_values: &[f64], target_times: &[f64], method: &Method, use_f64: bool) -> Result<Vec<f64>> {
		if input_times.is_empty() || target_times.is_empty() {
			return Ok(Vec::new());
		}
		...
		let output_size = target_times.len() * std::mem::size_of::<f64>();
		let output_buffer = self.device.create_buffer(&wgpu::BufferDescriptor { label: Some("Output Buffer"), size: output_size as u64, usage: BufferUsages::STORAGE | BufferUsages::COPY_SRC, mapped_at_creation: false });

		let staging_buffer = self.device.create_buffer(&wgpu::BufferDescriptor { label: Some("Staging Buffer"), size: output_size as u64, usage: BufferUsages::COPY_DST | BufferUsages::MAP_READ, mapped_at_creation: false });

		// Get or create pipeline
		let pipelines = if use_f64 { &mut self.pipelines_f64 } else { &mut self.pipelines_f32 };

		if !pipelines.contains_key(&method) {
			let shader_source = if use_f64 { method.shader_source_f64() } else { method.shader_source_f32() };
...

		{
			...
			let workgroup_size = 256;
			let num_workgroups = (target_times.len() + workgroup_size - 1) / workgroup_size;
			compute_pass.dispatch_workgroups(num_workgroups as u32, 1, 1);
		}
