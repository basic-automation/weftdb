...
use database::{
	splines::{gpu::test_gpu_availability, gpu_linear_interpolate_with_fallback, linear, Resolution}, Measurement
};
use fake::{Fake, Faker};
use uuid::Uuid;
...
fn gpu_vs_cpu_large_scale(c: &mut Criterion) {
	...
	for (measurement_count, output_points) in test_configs {
		...
		group.bench_with_input(BenchmarkId::new("cpu_linear", format!("{}k_to_{}k", measurement_count / 1000, output_points / 1000)), &(measurements.clone(), start, end), |b, (measurements, start, end)| {
			b.iter(|| black_box(linear(black_box(measurements.clone()), black_box(*start), black_box(*end), black_box(Resolution::Seconds)).unwrap()));
		});

		// GPU with optimized fallback benchmark
		group.bench_with_input(BenchmarkId::new("gpu_linear_optimized", format!("{}k_to_{}k", measurement_count / 1000, output_points / 1000)), &(measurements.clone(), start, end), |b, (measurements, start, end)| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter_custom(|iters| {
				...
				rt.block_on(async {
					for _ in 0..iters {
						...
						black_box(gpu_linear_interpolate_with_fallback(black_box(measurements.clone()), black_box(target_times), black_box(dataset_id)).await.unwrap());
					}
				});
				start_time.elapsed()
			});
...
fn gpu_crossover_analysis(c: &mut Criterion) {
	...
	for (measurement_count, output_points) in crossover_configs {
		...
		// CPU benchmark
		group.bench_with_input(BenchmarkId::new("cpu", &config_name), &(measurements.clone(), start, end), |b, (measurements, start, end)| {
			b.iter(|| black_box(linear(black_box(measurements.clone()), black_box(*start), black_box(*end), black_box(Resolution::Seconds)).unwrap()));
		});

		// GPU benchmark
		group.bench_with_input(BenchmarkId::new("gpu", &config_name), &(measurements.clone(), start, end), |b, (measurements, start, end)| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter_custom(|iters| {
				let measurements = measurements.clone();
				let start = *start;
...
fn gpu_output_scaling(c: &mut Criterion) {
	...
	let start = Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 0).unwrap();
	let measurements = generate_synthetic_measurements(fixed_input, start, start + chrono::Duration::hours(24));
...
fn gpu_spline_type_comparison(c: &mut Criterion) {
	...
	let output_points = 100_000;

	let start = Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 0).unwrap();
	let end = start + chrono::Duration::seconds(output_points as i64);
	let measurements = generate_synthetic_measurements(measurement_count, start, end);

	let spline_types = [("Linear", SplineType::Linear), ("Quadratic", SplineType::Quadratic), ("Cubic", SplineType::Cubic), ("Polynomial_2", SplineType::Polynomial(2)), ("Polynomial_3", SplineType::Polynomial(3)), ("Polynomial_5", SplineType::Polynomial(5))];
	...
	for (name, spline_type) in spline_types {
		...
		// CPU benchmark
		group.bench_with_input(BenchmarkId::new("cpu", name), &spline_type, |b, &spline_type| {
			b.iter(|| {
				black_box(
					match spline_type {
		...
		// GPU benchmark
		group.bench_with_input(BenchmarkId::new("gpu", name), &spline_type, |b, &spline_type| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter_custom(|iters| {
				let measurements = measurements.clone();
...
criterion_group!(benches, gpu_vs_cpu_large_scale, gpu_crossover_analysis, gpu_output_scaling, gpu_input_scaling, gpu_spline_type_comparison);
