...
use database::{
	auto_interpolate, splines::{gpu::test_gpu_availability, linear::linear}, Measurement, Resolution, SplineType
};
use fake::{Fake, Faker};
use uuid::Uuid;
...
fn check_gpu_availability() -> bool {
	...
	rt.block_on(async { test_gpu_availability().await.unwrap_or(false) })
}
...
fn gpu_vs_cpu_large_scale(c: &mut Criterion) {
	...
	for (measurement_count, output_points) in test_configs {
		...
		group.bench_with_input(BenchmarkId::new("gpu_auto", format!("{}k_to_{}k", measurement_count / 1000, output_points / 1000)), &(measurements.clone(), start, end), |b, (measurements, start, end)| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter(|| rt.block_on(async { black_box(auto_interpolate(black_box(measurements.clone()), black_box(*start), black_box(*end), black_box(Resolution::Seconds), black_box(SplineType::Linear)).await.unwrap()) }));
		});
	}
...
fn gpu_spline_type_comparison(c: &mut Criterion) {
	...
	let spline_types = [("Linear", SplineType::Linear), ("Quadratic", SplineType::Quadratic), ("Cubic", SplineType::Cubic)];
	...
	for (name, spline_type) in spline_types {
		...
		group.bench_with_input(BenchmarkId::new("gpu", name), &spline_type, |b, &spline_type| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter(|| rt.block_on(async { black_box(auto_interpolate(black_box(measurements.clone()), black_box(start), black_box(end), black_box(Resolution::Seconds), black_box(spline_type)).await.unwrap()) }));
		});
	}
