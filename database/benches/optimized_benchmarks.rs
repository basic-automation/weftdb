...
impl BenchmarkContext {
    async fn new(name: &str, measurement_count: usize) -> anyhow::Result<Self> {
        ...
        let measurements: Vec<_> = (0..measurement_count)
            .map(|i| InputMeasurement::new(
        ...
        c.bench_function(&format!("optimized_interpolation_{}", name), |b| {
            b.iter_batched(
                || {
                    ...
                    rt.block_on(async {
                        BenchmarkContext::new(name, size).await.unwrap()
                    })
                },
                |ctx| {
                    // Benchmark phase
...
fn benchmark_cache_efficiency(c: &mut Criterion) {
    ...
    c.bench_function("cache_hit_ratio", |b| {
        let ctx = rt.block_on(async {
            BenchmarkContext::new("cache_test", 100).await.unwrap()
        });
