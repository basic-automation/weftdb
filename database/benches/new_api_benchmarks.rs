        ...
        b.iter(|| {
            rt.block_on(async {
                let measurement = InputMeasurement::new(
                    Utc.with_ymd_and_hms(2023, 1, 1, 12, 0, 0).unwrap() + Duration::seconds(counter), 
                    BigDecimal::from_str("25.5").unwrap()
                );
    ...
    let aspect_id = rt.block_on(async {
        ...
        for i in 0..100 {
            let measurement = InputMeasurement::new(
                ...
                BigDecimal::from_str(&format!("{}.{}", i / 10, i % 10)).unwrap()
            );
            capture_measurement(aspect_id, measurement).await.unwrap();
        }
    ...
    let aspect_id = rt.block_on(async {
        ...
        for i in 0..1000 {
            let measurement = InputMeasurement::new(
                ...
                BigDecimal::from_str(&format!("{}.{}", i / 100, i % 100)).unwrap()
            );
            capture_measurement(aspect_id, measurement).await.unwrap();
        }
...
criterion_group!(
    ...
    benchmark_subject_creation, 
    benchmark_aspect_creation, 
    benchmark_measurement_capture, 
    benchmark_bulk_measurement_capture, 
    benchmark_point_analysis, 
    benchmark_range_analysis, 
