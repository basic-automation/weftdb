...
fn benchmark_optimization_strategies(c: &mut Criterion) {
	...
	for (name, measurement_count, window_minutes, resolution) in optimization_configs {
		...
		c.bench_function(name, |b| b.iter(|| rt.block_on(async { black_box(auto_interpolate(black_box(&mut measurements_to_points(&measurements.clone())), black_box(start), black_box(end), black_box(resolution), black_box(Spline::Linear)).await.unwrap()) })));
	}
}
