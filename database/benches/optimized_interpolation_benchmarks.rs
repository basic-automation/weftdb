...
fn bench_optimized_vs_standard_performance(c: &mut Criterion) {
    ...
    for &size in &test_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("standard", size), &size, |b, _| {
            b.iter(|| {
                auto_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(SplineType::Cubic))
            });
        });
        ...
        group.bench_with_input(BenchmarkId::new("optimized", size), &size, |b, _| {
            b.iter(|| {
                optimized_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(SplineType::Cubic))
            });
        });
    }
...
fn bench_fast_path_optimizations(c: &mut Criterion) {
    ...
    for (name, spline_type) in spline_types {
        ...
        group.bench_with_input(BenchmarkId::new("standard", name), &spline_type, |b, &spline_type| {
            b.iter(|| {
                auto_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(spline_type))
            });
        });
        ...
        group.bench_with_input(BenchmarkId::new("fast_path", name), &spline_type, |b, &spline_type| {
            b.iter(|| {
                fast_path_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(spline_type))
            });
        });
    }
    ...
    for &size in &test_sizes {
        ...
        for &chunk_size in &chunk_sizes {
            ...
            group.bench_with_input(BenchmarkId::new(format!("size_{}_chunk_{}", size, chunk_size), size), &size, |b, _| {
                b.iter(|| {
                    database::splines::streaming_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(SplineType::Linear), black_box(chunk_size))
                });
            });
        }
    }
...
fn bench_algorithm_selection_effectiveness(c: &mut Criterion) {
    ...
    for (size, requested_spline, name) in test_cases {
        ...
        group.bench_with_input(BenchmarkId::new("requested", name), &size, |b, _| {
            b.iter(|| {
                auto_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(requested_spline))
            });
        });
        ...
        group.bench_with_input(BenchmarkId::new("optimized", name), &size, |b, _| {
            b.iter(|| {
                optimized_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(requested_spline))
            });
        });
    }
...
fn bench_memory_usage_patterns(c: &mut Criterion) {
    ...
    group.bench_function("standard_memory", |b| {
        b.iter(|| {
            auto_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(SplineType::Linear))
        });
    });
    ...
    group.bench_function("optimized_memory", |b| {
        b.iter(|| {
            optimized_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(Resolution::Seconds), black_box(SplineType::Linear))
        });
    });
...
fn bench_resolution_impact(c: &mut Criterion) {
    ...
    for (name, resolution) in resolutions {
        group.bench_with_input(BenchmarkId::new("resolution", name), &resolution, |b, &resolution| {
            b.iter(|| {
                optimized_interpolate(black_box(measurements.clone()), black_box(interpolation_start), black_box(interpolation_end), black_box(resolution), black_box(SplineType::Quadratic))
            });
        });
    }
