...
fn benchmark_simd_linear(c: &mut Criterion) {
    ...
    for target_count in [32, 64, 128, 256, 512, 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("simd_batch", target_count), target_count, |b, _| {
            b.iter(|| linear_simd_batch(black_box(&measurements), black_box(&target_times)))
        });
    }
...
fn benchmark_simd_quadratic(c: &mut Criterion) {
    ...
    for target_count in [32, 64, 128, 256, 512, 1024].iter() {
        ...
        group.throughput(Throughput::Elements(*target_count as u64));
        group.bench_with_input(BenchmarkId::new("simd_batch", target_count), target_count, |b, _| {
            b.iter(|| quadratic_simd_batch(black_box(&measurements), black_box(&target_times)))
        });
    }
...
fn benchmark_simd_cubic(c: &mut Criterion) {
    ...
    for target_count in [32, 64, 128, 256, 512, 1024].iter() {
        ...
        group.throughput(Throughput::Elements(*target_count as u64));
        group.bench_with_input(BenchmarkId::new("simd_batch", target_count), target_count, |b, _| {
            b.iter(|| cubic_simd_batch(black_box(&measurements), black_box(&target_times)))
        });
    }
...
fn benchmark_simd_polynomial(c: &mut Criterion) {
    ...
    for degree in [2, 3, 4, 5].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("polynomial_degree", degree), degree, |b, &degree| {
            b.iter(|| polynomial_simd_batch(black_box(&measurements), black_box(&target_times), black_box(degree)))
        });
    }
...
fn benchmark_simd_vs_scalar(c: &mut Criterion) {
    ...
    // SIMD Linear
    group.bench_function("simd_linear", |b| {
        b.iter(|| linear_simd_batch(black_box(&measurements), black_box(&target_times)))
    });

    // Scalar Linear (via auto_interpolate)
    group.bench_function("scalar_linear", |b| {
        b.iter(|| database::splines::linear::linear(black_box(measurements.clone()), black_box(target_times[0]), black_box(target_times[target_times.len() - 1]), black_box(Resolution::Seconds)))
    });

    // SIMD Cubic
    group.bench_function("simd_cubic", |b| {
        b.iter(|| cubic_simd_batch(black_box(&measurements), black_box(&target_times)))
    });

    // Scalar Cubic
    group.bench_function("scalar_cubic", |b| {
        b.iter(|| database::splines::cubic::cubic(black_box(measurements.clone()), black_box(target_times[0]), black_box(target_times[target_times.len() - 1]), black_box(Resolution::Seconds)))
    });
...
fn benchmark_auto_interpolate_simd(c: &mut Criterion) {
    ...
    for (name, spline_type) in spline_types {
        group.bench_with_input(BenchmarkId::new("spline_type", name), &spline_type, |b, spline_type| {
            b.iter(|| auto_interpolate_simd(black_box(&measurements), black_box(&target_times), black_box(*spline_type)))
        });
    }
...
fn benchmark_simd_threshold_performance(c: &mut Criterion) {
    ...
    for target_count in [8, 16, 24, 32, 48, 64, 96, 128].iter() {
        ...
        group.throughput(Throughput::Elements(*target_count as u64));
        group.bench_with_input(BenchmarkId::new("target_count", target_count), target_count, |b, _| {
            b.iter(|| linear_simd_batch(black_box(&measurements), black_box(&target_times)))
        });
    }
...
fn benchmark_large_dataset_simd(c: &mut Criterion) {
    ...
    for measurement_count in [100, 500, 1000, 2000, 5000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("measurement_count", measurement_count), measurement_count, |b, _| {
            b.iter(|| linear_simd_batch(black_box(&measurements), black_box(&target_times)))
        });
    }
