...
fn benchmark_cache_miss_vs_hit(c: &mut Criterion) {
    ...
    let (db, aspect_id) = rt.block_on(async {
        ...
        for i in 0..100 { // Further reduced dataset size
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                let measurement = InputMeasurement::new(
                    ...
                    BigDecimal::from_str(&format!("{}.0", counter)).unwrap()
                );
                counter += 1;
...
fn benchmark_concurrent_cache_access(c: &mut Criterion) {
    ...
    let (db, aspect_id) = rt.block_on(async {
        ...
        // Add test data
        let base_time = Utc.with_ymd_and_hms(2023, 1, 1, 12, 0, 0).unwrap();
        for i in 0..60 { // Reasonable dataset
            let measurement = InputMeasurement::new(
                    ...
                    let tasks: Vec<_> = (0..concurrency)
                        .map(|i| {
                            ...
                            tokio::spawn(async move { 
                                db_clone.analyze_point(aspect_id, target_time, Resolution::Seconds, Spline::Linear).await.unwrap() 
                            })
                        })
                        .collect();
