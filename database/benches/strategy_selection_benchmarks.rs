...
use database::{
	splines::{auto_interpolate, Resolution, SplineType}, Measurement
};
use uuid::Uuid;
...
fn benchmark_strategy_selection(c: &mut Criterion) {
	...
	for (size, strategy_name) in dataset_sizes {
		...
		group.bench_with_input(BenchmarkId::new("dataset_size", strategy_name), &(measurements, start, end), |b, (measurements, start, end)| {
			b.iter(|| auto_interpolate(black_box(measurements.clone()), black_box(*start), black_box(*end), black_box(Resolution::Seconds), black_box(SplineType::Linear)));
		});
	}
...
fn benchmark_algorithm_complexity_impact(c: &mut Criterion) {
	...
	let spline_types = vec![(SplineType::Linear, "linear_complexity_1"), (SplineType::Quadratic, "quadratic_complexity_2"), (SplineType::Cubic, "cubic_complexity_3"), (SplineType::Polynomial(5), "polynomial_complexity_5")];
	...
	for (spline_type, name) in spline_types {
		group.bench_with_input(BenchmarkId::new("algorithm_complexity", name), &spline_type, |b, spline_type| {
			b.iter(|| auto_interpolate(black_box(measurements.clone()), black_box(start), black_box(end), black_box(Resolution::Seconds), black_box(*spline_type)));
		});
	}
...
fn benchmark_output_density_impact(c: &mut Criterion) {
	...
	for (duration, resolution, scenario_name) in density_scenarios {
		...
		group.bench_with_input(BenchmarkId::new("output_density", scenario_name), &(end, resolution), |b, (end, resolution)| {
...
fn benchmark_strategy_overhead(c: &mut Criterion) {
	...
	group.bench_function("direct_linear_call", |b| {
		b.iter(|| database::splines::linear(black_box(measurements.clone()), black_box(start), black_box(end), black_box(Resolution::Seconds)));
	});
...
fn benchmark_quick_strategy_verification(c: &mut Criterion) {
	...
	let scenarios = vec![(100, "small"), (1000, "medium"), (2000, "large")];
