...
impl CubicSpline {
	...
	fn create_natural_cubic_spline(measurements: &[Measurement]) -> Result<Self> {
		...
		for i in 0..n - 1 {
			let x0 = BigDecimal::from_i64(measurements[i].timestamp.timestamp_millis()).ok_or_else(|| anyhow::anyhow!("Failed to convert timestamp to BigDecimal"))?;
			let y0 = measurements[i].value.clone();
			let x1 = BigDecimal::from_i64(measurements[i + 1].timestamp.timestamp_millis()).ok_or_else(|| anyhow::anyhow!("Failed to convert timestamp to BigDecimal"))?;
	...
	pub fn evaluate(&self, target_time: DateTime<Utc>) -> Result<BigDecimal> {
		...
		if target_time >= self.measurements[n - 1].timestamp {
			...
			let dt = BigDecimal::from_i64(duration.num_milliseconds()).ok_or_else(|| anyhow::anyhow!("Failed to convert timestamp difference to BigDecimal for interpolation"))?;
...
pub fn cubic(measurements: Vec<Measurement>, start: DateTime<Utc>, end: DateTime<Utc>, resolution: Resolution) -> Result<Vec<Measurement>> {
	...
	while current <= end {
		...
		results.push(Measurement { id: Uuid::new_v4(), dataset_id, timestamp: current, value });
		current += step;
	}
...
pub fn cubic_parallel_dense(
	...
	let results: Result<Vec<Measurement>, _> = target_times
		...
		.map(|&time| {
			...
			Ok(Measurement { id: Uuid::new_v4(), dataset_id, timestamp: time, value })
		})
		.collect();
