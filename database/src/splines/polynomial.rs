...
use splines::{polynomial, polynomial_simd, Resolution};
...
pub fn polynomial(measurements: Vec<Measurement>, start: DateTime<Utc>, end: DateTime<Utc>, resolution: Resolution, degree: usize) -> Result<Vec<Measurement>> {
	...
	let x_values: Vec<f64> = measurements.iter().map(|m| (m.timestamp - base_time).num_seconds() as f64).collect();
	let y_values: Vec<f64> = measurements.iter().map(|m| m.value.to_string().parse::<f64>().unwrap_or(0.0)).collect();
	...
	for target_time in target_times {
		...
		let mut distances: Vec<(usize, f64)> = x_values.iter().enumerate().map(|(i, &x)| (i, (x - target_x).abs())).collect();
		distances.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());

		let indices: Vec<usize> = distances.into_iter().take(max_points).map(|(i, _)| i).collect();
		...
		let value = bigdecimal::BigDecimal::from_str(&result.to_string()).context("Failed to convert interpolated value to BigDecimal")?;

		results.push(Measurement { id: uuid::Uuid::new_v4(), dataset_id, timestamp: target_time, value });
	}
