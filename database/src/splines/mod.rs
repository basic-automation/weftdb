...
pub mod polynomial;
pub mod quadratic;
pub mod simd; // Add simd module // Add parallel module
...
pub use parallel::{fast_path_interpolate, optimized_interpolate};
...
#[must_use]
pub fn apply_fast_path_optimization(spline_type: SplineType, measurement_count: usize, _complexity_factor: usize) -> SplineType {
	match spline_type {
...
pub fn auto_interpolate(measurements: Vec<Measurement>, start: DateTime<Utc>, end: DateTime<Utc>, resolution: Resolution, spline_type: SplineType) -> Result<Vec<Measurement>> {
	...
	let params = InterpolationParams { measurements, start, end, resolution, spline_type: optimized_spline_type };
...
pub async fn auto_interpolate_async(measurements: Vec<Measurement>, start: DateTime<Utc>, end: DateTime<Utc>, resolution: Resolution, spline_type: SplineType) -> Result<Vec<Measurement>> {
	...
	// Use optimized GPU thresholds based on spline type and benchmark results
	let use_gpu = match spline_type {
		SplineType::Linear => should_use_gpu_interpolation(measurements.len(), estimated_output_points),
		SplineType::Quadratic => quadratic::should_use_gpu_quadratic(measurements.len(), estimated_output_points),
		SplineType::Cubic => cubic::should_use_gpu_cubic(measurements.len(), estimated_output_points),
		SplineType::Polynomial(degree) => polynomial::should_use_gpu_polynomial(measurements.len(), estimated_output_points, degree),
	};
	...
	if use_gpu {
		...
		match spline_type {
			SplineType::Linear => {
				let target_times = generate_target_times(start, end, resolution);
				gpu::gpu_linear_interpolate_with_fallback(measurements, target_times, dataset_id).await
			}
			SplineType::Quadratic => quadratic::gpu_quadratic_interpolate_with_fallback(measurements, start, end, resolution, dataset_id).await,
			SplineType::Cubic => cubic::gpu_cubic_interpolate_with_fallback(measurements, start, end, resolution, dataset_id).await,
			SplineType::Polynomial(degree) => polynomial::gpu_polynomial_interpolate_with_fallback(measurements, start, end, resolution, dataset_id, degree).await,
		}
	} else {
		// Use CPU for smaller datasets
		match spline_type {
			SplineType::Linear => linear::linear(measurements, start, end, resolution),
			SplineType::Quadratic => quadratic::quadratic(measurements, start, end, resolution),
			SplineType::Cubic => cubic::cubic(measurements, start, end, resolution),
			SplineType::Polynomial(degree) => polynomial::polynomial(measurements, start, end, resolution, degree),
		}
	}
}
