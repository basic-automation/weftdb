...
impl Database {
	...
	pub async fn observe_measurements_batch(&self, aspect: Aspect, measurements: Vec<InputMeasurement>) -> Result<Vec<TxId>> {
		...
		let f = DATABASES.lock().await.values()
			.find_map(|db_info| db_info.subjects().values()
				.find_map(|subject_info| subject_info.aspects().get(&aspect.id())
					.map(|aspect_info| (subject_info.pool(), aspect_info))))
			.map(|(pool, aspect_info)| (pool.clone(), aspect_info.table_name().to_string()));
		let Some((pool, table_name)) = f else { 
			bail!(Error::DatabaseError("Aspect not found".to_string())) 
		};
		...
		for measurement in measurements {
			...
			sqlx::query(&insert_sql)
				.bind(tx_id.as_uuid().to_string())
				.bind(measurement.timestamp().timestamp_millis())
				.bind(measurement.value().to_string())
				.execute(&mut *tx).await
				.map_err(|e| Error::DatabaseError(format!("Failed to insert measurement: {e}")))?;
		}
