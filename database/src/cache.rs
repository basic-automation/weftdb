...
impl DatabaseCache {
    ...
    pub async fn stats(&self) -> CacheStats {
        ...
        CacheStats {
            measurement_entries: measurements.len(),
            point_analysis_entries: point_analysis.len(),
            range_analysis_entries: range_analysis.len(),
            total_measurements: measurements.values().map(std::vec::Vec::len).sum(),
            total_range_points: range_analysis.values().map(std::vec::Vec::len).sum(),
        }
    }
}
