...
async fn test_database_lifecycle() {
    ...
    let measurements = vec![
        InputMeasurement::new(
            ...
            BigDecimal::from_str("20.5").unwrap()
        ),
        InputMeasurement::new(
            Utc.with_ymd_and_hms(2023, 1, 1, 12, 5, 0).unwrap(),
            BigDecimal::from_str("21.0").unwrap()
        ),
        InputMeasurement::new(
            Utc.with_ymd_and_hms(2023, 1, 1, 12, 10, 0).unwrap(),
            BigDecimal::from_str("21.5").unwrap()
        ),
    ];
    ...
    let data_point = analyze_point(aspect_id, target_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze point");
    ...
    let range_data = analyze_range(aspect_id, start_time, end_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze range");
...
async fn test_existing_database() {
    ...
    capture_measurement(
        ...
        InputMeasurement::new(
            Utc.with_ymd_and_hms(2023, 1, 1, 12, 0, 0).unwrap(),
            BigDecimal::from_str("45.0").unwrap()
        )
    ).await.expect("Failed to capture measurement");
...
async fn test_multiple_subjects_and_aspects() {
    ...
    for i in 0..5 {
        ...
        capture_measurement(
            ...
            InputMeasurement::new(
                timestamp,
                BigDecimal::from_str(&format!("{}.0", 20 + i)).unwrap()
            )
        ).await.expect("Failed to capture temp measurement for subject 1");
        ...
        capture_measurement(
            ...
            InputMeasurement::new(
                timestamp,
                BigDecimal::from_str(&format!("{}.0", 40 + i)).unwrap()
            )
        ).await.expect("Failed to capture humidity measurement for subject 1");
        ...
        capture_measurement(
            ...
            InputMeasurement::new(
                timestamp,
                BigDecimal::from_str(&format!("{}.0", 15 + i)).unwrap()
            )
        ).await.expect("Failed to capture temp measurement for subject 2");
    }
    
    // Test analysis on different aspects
    let analysis_time = base_time + chrono::Duration::minutes(10);
    
    let temp1_result = analyze_point(temp_aspect1, analysis_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze temperature for subject 1");
    
    let humidity1_result = analyze_point(humidity_aspect1, analysis_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze humidity for subject 1");
    
    let temp2_result = analyze_point(temp_aspect2, analysis_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze temperature for subject 2");
...
async fn test_interpolation_methods() {
    ...
    let linear_result = analyze_point(aspect_id, linear_target, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze with linear interpolation");
    
    let quadratic_result = analyze_point(aspect_id, quadratic_target, Resolution::Seconds, SplineType::Quadratic)
        .await
        .expect("Failed to analyze with quadratic interpolation");
    
    let cubic_result = analyze_point(aspect_id, cubic_target, Resolution::Seconds, SplineType::Cubic)
        .await
        .expect("Failed to analyze with cubic interpolation");
    ...
    let linear_result2 = analyze_point(aspect_id, linear_target, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze with linear interpolation (second call)");
...
async fn test_caching_behavior() {
    ...
    let result1 = analyze_point(aspect_id, target_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze point (first call)");
    let first_duration = start.elapsed();
    
    // Second call should be faster due to caching
    let start = std::time::Instant::now();
    let result2 = analyze_point(aspect_id, target_time, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze point (second call)");
