...
use database::{add_subject, analyze_point, capture_measurement, new, track_aspect, AspectId, DatabaseId, InputMeasurement, SubjectId, DEFAULT_DATA_DIR};
...
async fn test_create_btc_1min_database() -> Result<()> {
	...
	let db_id = new("crypto").await?;
	let subject_id = add_subject(db_id, "Bitcoin").await?;
	let aspect_id = track_aspect(subject_id, "price_from_kaggle").await?;
