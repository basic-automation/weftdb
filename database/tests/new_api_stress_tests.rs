...
async fn test_multiple_aspects_same_subject() {
    ...
    for i in 0..num_aspects {
        let aspect_id = track_aspect(subject_id, &format!("aspect_{}", i))
            .await
            .expect("Failed to track aspect");
    ...
    for (aspect_idx, &aspect_id) in aspect_ids.iter().enumerate() {
        for i in 0..100i64 { // Make i explicitly i64
            let measurement = InputMeasurement::new(
                base_time + Duration::minutes(i),
                BigDecimal::from_str(&format!("{}.{}", aspect_idx * 10 + (i as usize % 10), i % 100)).unwrap()
            );
            capture_measurement(aspect_id, measurement).await
                .expect("Failed to capture multi-aspect measurement");
        }
    }
    ...
    for &aspect_id in &aspect_ids {
        let result = analyze_point(aspect_id, target_time, Resolution::Seconds, SplineType::Linear)
            .await
            .expect("Failed to analyze multi-aspect point");
...
async fn test_high_frequency_measurements() {
    ...
    for i in 0..num_measurements {
        let measurement = InputMeasurement::new(
            ...
            BigDecimal::from_str(&format!("{}.{}", (i % 100), (i % 10))).unwrap()
        );
        
        capture_measurement(aspect_id, measurement).await.expect("Failed to capture measurement");
        
        if i % 100 == 0 { // Log progress every 100 measurements
            println!("Captured {} measurements", i);
        }
    }
    
    let capture_duration = start.elapsed();
    println!("Captured {} measurements in {:?} ({:.2} measurements/sec)", 
             num_measurements, capture_duration, num_measurements as f64 / capture_duration.as_secs_f64());
...
async fn test_concurrent_access() {
    ...
    let base_time = Utc.with_ymd_and_hms(2023, 1, 1, 12, 0, 0).unwrap();
    for i in 0..60 { // Reduced initial data
        let measurement = InputMeasurement::new(
            base_time + Duration::minutes(i),
    ...
    let tasks: Vec<_> = (0..num_concurrent_tasks).map(|task_id| {
        ...
        tokio::spawn(async move {
            for op_id in 0..operations_per_task {
                ...
                if op_id % 2 == 0 {
                    // Write operation
                    let measurement = InputMeasurement::new(
                        base_time + Duration::minutes(100 + task_id * operations_per_task + op_id),
                        BigDecimal::from_str(&format!("{}.{}", task_id, op_id)).unwrap()
                    );
                    ...
                    let _result = analyze_point(aspect_id, target_time, Resolution::Seconds, SplineType::Linear)
                        .await
                        .expect("Failed to analyze concurrent point");
                }
            }
            task_id as i64
        })
    ...
    ).await.expect("Concurrent test timed out");
...
async fn test_large_dataset_analysis() {
    ...
    for i in 0..num_measurements {
        ...
        let measurement = InputMeasurement::new(
            timestamp,
            BigDecimal::from_str(&format!("{:.2}", value)).unwrap()
        );
    ...
    for (spline_type, name) in test_cases {
        ...
        let result = analyze_point(aspect_id, target_time, Resolution::Seconds, spline_type)
            .await
            .expect("Failed to analyze large dataset point");
...
async fn test_memory_usage_stability() {
    ...
    for cycle in 0..5 { // Reduced from 10
        println!("Memory test cycle {}/5", cycle + 1);
        
        // Add batch of measurements
        for i in 0..100 { // Reduced from 500 to prevent time range issues
            let measurement = InputMeasurement::new(
        ...
        for i in 0..10 { // Reduced from 50
            ...
            let _result = analyze_point(aspect_id, target_time, Resolution::Seconds, SplineType::Linear)
                .await
                .expect("Failed to analyze memory test point");
        }
...
async fn test_edge_case_scenarios() {
    ...
    for (seconds_offset, value) in sparse_measurements {
        let measurement = InputMeasurement::new(
            base_time + Duration::seconds(seconds_offset),
    ...
    let sparse_result = analyze_point(aspect_id, sparse_target, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze sparse data");
    ...
    let dense_result = analyze_point(aspect_id, dense_target, Resolution::Seconds, SplineType::Cubic)
        .await
        .expect("Failed to analyze dense data");
    ...
    for (seconds_offset, value) in extreme_measurements {
        let measurement = InputMeasurement::new(
            extreme_base + Duration::seconds(seconds_offset),
            BigDecimal::from_str(value).unwrap()
        );
    ...
    let extreme_result = analyze_point(aspect_id, extreme_target, Resolution::Seconds, SplineType::Linear)
        .await
        .expect("Failed to analyze extreme data");
