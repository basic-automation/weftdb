...
use database::{
	splines::{auto_interpolate, gpu::test_gpu_availability}, Measurement, Resolution, SplineType
};
use uuid::Uuid;
...
async fn test_gpu_vs_cpu_consistency() {
	...
	let cpu_result = auto_interpolate(measurements.clone(), start, end, Resolution::Seconds, SplineType::Linear).await;
...
async fn test_large_dataset_gpu_threshold() {
	...
	let result = auto_interpolate(measurements, start, end, Resolution::Milliseconds, SplineType::Linear).await;
