...
impl ThorchainNinerealms {
	...
	pub async fn get_usd_quote(&self, pool: &str, interval: DepthInterval, count: u16, to: Option<i64>, from: Option<i64>) -> Vec<(i64, f64)> {
		let client = reqwest::Client::new();
		...
		let mut quotes: Vec<(i64, f64)> = Vec::new();
		for interval in response["intervals"].as_array().unwrap() {
			let price = f64::from_str(interval["assetPriceUSD"].as_str().unwrap()).unwrap();
