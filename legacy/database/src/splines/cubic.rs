...
impl CubicSpline {
	fn new(measurements: &[Measurement]) -> Result<Self> {
		...
		if num_points == 2 {
			...
			let intervals: Result<Vec<BigDecimal>> = (0..num_points - 1).map(|i| BigDecimal::from_i64((measurements[i + 1].timestamp - measurements[i].timestamp).num_milliseconds()).context(format!("Failed to convert timestamp difference to BigDecimal for segment {i}"))).collect();
			let intervals = intervals?;

			let delta: Vec<BigDecimal> = (0..num_points - 1).map(|i| (measurements[i + 1].value.clone() - measurements[i].value.clone()) / intervals[i].clone()).collect();
