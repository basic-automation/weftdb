...
//! - `weft-arrow` depends only on `weft-physical-type` and *never* the reverse, so it
...
//! tree stays out of `database`/`splimes`/`weft-physical-type`.
//!
...
//! (hard constraint #4). A range that spans several `.weftseg` segments of
...
/// [`weft_arrow::columns_to_record_batch`], tagged with the aspect's declared
/// [`TimeUnit`].
///
/// # Errors
///
...
/// prune, filesystem read, or a corrupt `.weftseg` frame).
pub async fn read_time_range_to_record_batch(store: &SegmentStore, aspect: &str, start: i64, end: i64) -> Result<RecordBatch> {
	...
	Ok(weft_arrow::columns_to_record_batch(unit, &timestamps, &values))
}
...
/// segments ([`AspectSchema::value`](weft_physical_type::AspectSchema)), the whole
/// logical range shares it, so a single typed Arrow column is well-defined. An
/// [`F64`](weft_physical_type::PhysicalType::F64) aspect yields a `Float64` column,
/// a fixed-scale [`ScaledI64`](weft_physical_type::PhysicalType::ScaledI64) /
/// [`ScaledI128`](weft_physical_type::PhysicalType::ScaledI128) aspect an **exact**
...
pub async fn read_time_range_to_record_batch_typed(store: &SegmentStore, aspect: &str, start: i64, end: i64) -> Result<RecordBatch> {
	...
	let (timestamps, values) = store.read_time_range(aspect, start, end).await?;
	Ok(weft_arrow::columns_to_record_batch_typed(schema.timestamp_unit, schema.value, &timestamps, &values))
}
...
/// with [`weft_arrow::write_ipc_stream`]. The stream is self-describing (the
/// time-unit and physical-encoding metadata travel in its schema), so a consumer
/// recovers the columns with [`weft_arrow::read_ipc_stream`] +
/// [`weft_arrow::record_batches_to_columns`] and nothing else. An empty window still
...
/// failure), plus a [`weft_arrow::ConvertError`] if IPC serialization fails.
pub async fn read_time_range_to_ipc_bytes(store: &SegmentStore, aspect: &str, start: i64, end: i64) -> Result<Vec<u8>> {
	...
	Ok(weft_arrow::write_ipc_stream(std::slice::from_ref(&batch))?)
}
...
/// # Errors
///
/// As [`read_value_range_to_record_batch`], plus a [`weft_arrow::ConvertError`] if
...
/// the batch with [`weft_arrow::write_parquet`]. The file is self-describing (the
/// time-unit and physical-encoding metadata travel in the embedded Arrow schema),
/// so a consumer recovers the columns with [`weft_arrow::read_parquet`] +
/// [`weft_arrow::record_batches_to_columns`] and nothing else — or loads it straight
...
///
/// As [`read_time_range_to_record_batch_typed`] (undeclared aspect, or a read
/// failure), plus a [`weft_arrow::ConvertError`] if Parquet serialization fails.
pub async fn read_time_range_to_parquet_bytes(store: &SegmentStore, aspect: &str, start: i64, end: i64) -> Result<Vec<u8>> {
	let batch = read_time_range_to_record_batch_typed(store, aspect, start, end).await?;
	Ok(weft_arrow::write_parquet(std::slice::from_ref(&batch))?)
}
...
/// [`weft_arrow::read_parquet`], recovers the logical `(timestamp, value)` columns
/// with [`weft_arrow::record_batches_to_columns`], and seals them into the store
...
/// are not a valid Parquet file or carry no WeftDB columns (a
/// [`weft_arrow::ConvertError`]), if `require_sorted` is set and the timestamps step
...
pub async fn ingest_parquet_into_aspect(store: &SegmentStore, aspect: &str, bytes: &[u8], rows_per_page: Option<usize>, require_sorted: bool) -> Result<SegmentDescriptor> {
	let schema = aspect_schema(store, aspect).await?;
	let batches = weft_arrow::read_parquet(bytes)?;
	let (timestamps, values) = weft_arrow::record_batches_to_columns(&batches)?;
	if require_sorted {
...
/// Resolve the full declared [`AspectSchema`](weft_physical_type::AspectSchema) for
...
mod tests {
	...
	async fn typed_time_range_read_emits_a_float64_column() {
		...
		use weft_arrow::{META_VALUE_ENCODING, VALUE_ENCODING_F64};
	...
	async fn time_range_to_ipc_bytes_round_trips_through_the_wire_form() {
		...
		// A consumer with only weft-arrow recovers the columns from the bytes.
	...
	async fn empty_window_to_ipc_bytes_is_a_valid_zero_row_stream() {
		use weft_arrow::read_ipc_stream;
	...
	async fn time_range_to_parquet_bytes_round_trips_through_the_file_form() {
		use weft_arrow::{read_parquet, record_batches_to_columns};
		...
		// A consumer with only weft-arrow recovers the columns from the file.
	...
	async fn parquet_ingest_into_undeclared_aspect_errors_with_no_declared_schema() {
		use weft_arrow::{columns_to_record_batch, write_parquet};
