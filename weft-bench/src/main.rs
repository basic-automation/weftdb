//! `weft-bench` — the command-line runner for Weft-Bench.
//!
//! Up to now the line-protocol → interpolation path existed only at the library
//! level: [`weft_bench::InterpolationProfile::from_line_protocol`] could turn a
...
//! the chosen numeric field through the same WeftDB interpolation harness, correctness
//! gate, and reporting as every other Weft-Bench run, and persists the result as a
//! durable `reports/json/` artifact (the "keep raw results" rule).
//!
//! ```text
//! weft-bench --input data.lp --field usage \
...
async fn run(cli: Cli) -> anyhow::Result<ExitCode> {
	...
	// a seeded corpus and times WeftDB's streaming point read, so it builds its own
	...
	let weft_result = run_profile(&WeftAdapter::new(), &profile, cli.reps).await.map_err(|e| anyhow::anyhow!("WeftDB benchmark run failed: {e}"))?;
	results.push(weft_result);
	if cli.compare {
...
/// point-lookup CLI knobs, seal a columnar segment, time WeftDB's streaming point read
...
/// range-fetch CLI knobs, seal a columnar segment, time WeftDB's windowed range read
...
/// downsample CLI knobs, generate a dense series, and time WeftDB's canonical reduction
/// (`weft-reduce`) into grid-aligned buckets. No analytic ground truth — correctness
...
fn finish(cli: &Cli, report: &BenchReport) -> anyhow::Result<ExitCode> {
	// The artifact name tags every adapter in the report (e.g. `weftdb+baseline-linear`)
...
fn print_summary(report: &BenchReport, out_path: &std::path::Path) {
	println!("Weft-Bench run complete");
...
const USAGE: &str = "\
weft-bench — run a WeftDB interpolation benchmark over a line-protocol file or the
seeded synthetic generator

USAGE:
    weft-bench --input <FILE.lp> --field <NAME> [OPTIONS]
    weft-bench <FILE.lp> --field <NAME> [OPTIONS]
    weft-bench --synthetic [OPTIONS]
    weft-bench --point-lookup [POINT-LOOKUP OPTIONS]
...
INPUT MODE (choose one):
    ...
    -p, --point-lookup       Run the storage-backed point-lookup workload instead of
                             ...
                             WeftDB's streaming point read (p50/p95/p99 + bytes/point).
    ...
    -r, --range-fetch        Run the storage-backed range-fetch workload: seal a
                             seeded columnar segment and time WeftDB's windowed range
...
`__weftdb.json`, or `__weftdb+baseline-linear+baseline-forward-fill.json` under
