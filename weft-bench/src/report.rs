//! JSON report artifacts for Weft-Bench runs.
//!
...
pub struct RunMetadata {
	/// Version of the `weft-bench` crate that produced the report.
	pub weft_bench_version: String,
...
impl BenchReport {
	...
	pub fn to_html(&self) -> String {
		...
		let version = escape_html(&m.weft_bench_version);
...
/// e.g. `interpolation-heavy-irregular__weftdb.json`.
///
...
/// pair, e.g. `interpolation-heavy-irregular__weftdb.html` — the HTML sibling of
/// [`default_filename`].
#[must_use]
...
mod tests {
	...
	fn most_accurate_picks_the_lowest_finite_rmse_publishable_result() {
		let report = BenchReport::with_results(metadata(), vec![result_with_rmse("weftdb", 1.9), result_with_rmse("baseline-linear", 1.2), result_with_rmse("baseline-forward-fill", 2.9)]);
	...
	fn most_accurate_ignores_results_without_accuracy_or_failing_correctness() {
		...
		let mut failing = result_with_rmse("weft-broken", 0.1);
		failing.correctness.values_finite = false; // makes it non-publishable
		let report = BenchReport::with_results(metadata(), vec![sample_result("ilp-weftdb", true), failing, result_with_rmse("baseline-linear", 1.2)]);
	...
	fn most_accurate_is_none_when_no_publishable_result_has_accuracy() {
		...
		let report = BenchReport::with_results(metadata(), vec![sample_result("weftdb", true), sample_result("baseline-linear", true)]);
	...
	fn capture_fills_version_and_target() {
		...
		assert_eq!(meta.weft_bench_version, env!("CARGO_PKG_VERSION"));
		assert_eq!(meta.os, std::env::consts::OS);
		assert_eq!(meta.arch, std::env::consts::ARCH);
		assert!(!meta.weft_bench_version.is_empty());
	}
	...
	fn to_html_surfaces_captured_hardware() {
		...
		let report = BenchReport::with_results(metadata(), vec![sample_result("weftdb", true)]);
	...
	fn report_round_trips_through_json() {
		let report = BenchReport::with_results(metadata(), vec![sample_result("weftdb", true), sample_result("duckdb", true)]);
	...
	fn publishability_requires_all_results_to_pass() {
		...
		report.push(sample_result("weftdb", true));
	...
	fn write_json_creates_parents_and_round_trips_from_disk() {
		...
		dir.push(format!("weft-bench-report-test-{}", std::process::id()));
		dir.push("json");
		let path = dir.join(default_filename("interpolation-heavy-irregular", "weftdb"));
	...
	fn default_filename_is_sanitized() {
		assert_eq!(default_filename("interpolation-heavy-irregular", "weftdb"), "interpolation-heavy-irregular__weftdb.json");
	...
	fn default_html_filename_mirrors_json_with_an_html_extension() {
		assert_eq!(default_html_filename("interpolation-heavy-irregular", "weftdb"), "interpolation-heavy-irregular__weftdb.html");
	...
	fn to_html_renders_a_document_with_one_row_per_result() {
		let report = BenchReport::with_results(metadata(), vec![result_with_rmse("weftdb", 1.9), result_with_rmse("baseline-linear", 1.2)]);
		let html = report.to_html();
		assert!(html.starts_with("<!doctype html>"), "must be a full HTML document");
		assert!(html.contains("Weft-Bench report"));
		// Metadata is surfaced in the header.
		assert!(html.contains("testos/testarch"), "metadata must appear: {html}");
		// Every adapter gets a row.
		assert!(html.contains("<td>weftdb</td>"), "weftdb row missing");
	...
	fn to_html_renders_the_workload_column_per_row() {
		...
		let mut point_lookup = sample_result("weftdb", true);
		...
		assert!(html.contains("<td>weftdb</td><td>point_lookup</td>"), "row must render its workload: {html}");
	}
	...
	fn write_html_creates_parents_and_writes_a_document() {
		let report = BenchReport::with_results(metadata(), vec![result_with_rmse("weftdb", 1.2)]);
		let mut dir = std::env::temp_dir();
		dir.push(format!("weft-bench-html-test-{}", std::process::id()));
		dir.push("html");
		let path = dir.join(default_html_filename("interpolation-heavy-irregular", "weftdb"));
		...
		assert!(raw.contains("<td>weftdb</td>"));
