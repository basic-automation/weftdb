...
//! classic TSDB aggregation workload the roadmap lists for Phase 1. This drives WeftDB's
//! canonical reduction — the vendor-neutral [`weft_reduce`] crate the HTTP
...
//! bucket holds many samples, and the workload asks [`weft_reduce::reduce`] to collapse
...
pub struct DownsampleProfile {
	...
	pub aggregations: Vec<Aggregation>,
	/// Reduce the input in this many parallel chunks via `weft_reduce::reduce_partial` +
	/// `PartialReduction::merge`, instead of one serial `reduce` pass. `0` or `1` keeps
	/// the serial path. The merged result is *exactly* the serial one (weft-reduce proves
...
/// # Errors
///
/// Propagates any [`weft_reduce::ReduceError`] from the reduction or the merge.
fn reduce_points(points: &[Point], profile: &DownsampleProfile) -> Result<Vec<Bucket>, weft_reduce::ReduceError> {
...
/// re-reduces the whole series via [`weft_reduce::reduce`] and the timed span covers
