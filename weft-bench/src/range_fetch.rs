...
//! **windowed range read**: `weftseg::read_segment_range` returns only the rows in a
...
//! a Weft-Bench workload (`range_fetch`), the roadmap's pending "raw range fetch"
...
/// seals it into a `.weftseg` segment and fetches `window_count` selective
...
/// rep re-fetches the whole window set against the sealed `.weftseg` bytes and the
