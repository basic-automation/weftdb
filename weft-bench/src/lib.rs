//! # Weft-Bench
//!
//! A reproducible, correctness-gated benchmark harness for WeftDB, the first track
//! on the roadmap's benchmark-led commercial thesis. Weft-Bench is both an
...
//! - [`weft_adapter`] — the WeftDB reference adapter ([`WeftAdapter`]).
//! - [`baseline_adapter`] — the portable client-side linear baseline
//!   ([`BaselineLinearAdapter`]) WeftDB is compared against (fair-protocol class C).
...
pub async fn run_profile<A: SystemAdapter + ?Sized>(adapter: &A, profile: &InterpolationProfile, reps: usize) -> anyhow::Result<BenchResult> {
	...
	let storage = Some(StorageEstimate::from_columns(&stored_values, &stored_timestamps, weft_physical_type::TimeUnit::Micros, &bigdecimal::BigDecimal::from(0)));
...
mod tests {
	...
	#[tokio::test]
	async fn weft_adapter_runs_interpolation_heavy_irregular() {
		let profile = InterpolationProfile::interpolation_heavy_irregular();
		let adapter = WeftAdapter::new();
		...
		// The WeftDB path must produce a non-empty, finite, correctly-sized grid.
	...
	async fn run_result_persists_as_a_json_report_artifact() {
		...
		assert!(report.is_publishable(), "a passing WeftDB run yields a publishable report");
		...
		path.push(format!("weft-bench-e2e-{}", std::process::id()));
		...
		assert_eq!(back.results[0].adapter, "weftdb");
	...
	#[tokio::test]
	async fn weft_adapter_runs_a_line_protocol_sourced_profile() {
...
cpu,host=h0 usage=14.0 600\n";
	...
	async fn measure_accuracy_scores_every_reconstruction_against_ground_truth() {
		...
		let weftdb = measure_accuracy(&WeftAdapter::new(), &profile).await.expect("weftdb accuracy");
		...
		for (label, m) in [("weftdb", weftdb), ("linear", linear), ("forward-fill", locf)] {
	...
	async fn noise_free_data_reconstructs_far_more_accurately_than_noisy() {
		...
		// This is the regime where WeftDB's spline path is meant to shine.
		...
		let clean_acc = measure_accuracy(&WeftAdapter::new(), &clean).await.expect("clean accuracy");
		let noisy_acc = measure_accuracy(&WeftAdapter::new(), &noisy).await.expect("noisy accuracy");
	...
	async fn measure_accuracy_rejects_a_line_protocol_profile() {
		...
		let err = measure_accuracy(&WeftAdapter::new(), &profile).await.expect_err("no ground truth must error");
	...
	async fn run_profile_records_the_signal_shape_and_accuracy_for_every_shape() {
		...
		for shape in [SignalShape::MultiSine, SignalShape::Sawtooth, SignalShape::Step, SignalShape::DampedSine] {
			...
			let result = run_profile(&WeftAdapter::new(), &profile, 3).await.expect("run completes");
