...
use ::weftdb::database::traits::{AspectStructure, Inputs, Outputs};
use anyhow::Result;
...
async fn debug_batch_processing() -> Result<()> {
	...
	let unprocessed_batches: Vec<weftdb::Batch> = db.get_unprocessed_batches(&test_aspect.id()).await?.try_collect().await?;
