...
//! and the typed `.weftseg` seal path. The declared `value_tolerance` is honoured by
...
/// actually runs is counted in `weft_reconcile_passes_total` (holds below the
...
/// (point `WEFT_SEGMENT_STORE_ROOT` at the restored copy and restart), not an HTTP call.
///
/// # Errors
///
...
pub struct IngestResponse {
	...
	pub segment_id: u64,
	/// The `.weftseg` frame format version sealed (5 single-block, 6 paged).
...
/// ([`weft_physical_type::first_order_violation`] / `AspectSchema::seal_sorted`).
///
...
async fn ingest_points_inner(store: &weftdb::SegmentStore, metrics: &crate::metrics::SharedMetrics, aspect: &str, request: IngestRequest) -> Result<Response, StorageError> {
	...
	// `.weftseg` write + control-plane index update — the write stage timed apart from
	// value parsing.
...
/// `weft-line-protocol` crate, so the storage ingest path and the interpolation ILP
...
async fn ingest_ilp_inner(store: &weftdb::SegmentStore, metrics: &crate::metrics::SharedMetrics, aspect: &str, params: &IlpIngestParams, body: &str) -> Result<Response, StorageError> {
	...
	let points = tracing::info_span!("storage.ingest.parse", format = "ilp", field = %params.field).in_scope(|| weft_line_protocol::parse_points(body, &params.field, precision)).map_err(|err| StorageError::BadRequest(err.to_string()))?;
	if points.is_empty() {
...
mod tests {
	...
	async fn router_with_declared_price(dir: &TempDir) -> axum::Router {
		let store = Arc::new(SegmentStore::open(dir.path()).await.expect("opens store"));
		store.declare("price", &weft_physical_type::AspectSchema::new(weft_physical_type::PhysicalType::F64, "0".parse().unwrap(), weft_physical_type::TimeUnit::Seconds)).await.expect("declares");
		app_with_state(AppState::new().with_store(store))
	}
	...
	async fn backup_endpoint_snapshots_control_plane_and_guards_label() {
		...
		assert!(mtext.contains("weft_backup_snapshots_total 1"), "one snapshot recorded; got:\n{mtext}");
		assert!(mtext.contains("weft_backup_bytes_written_total"), "bytes-written counter present");
	...
	async fn ingest_paged_batch_seals_paged_frame() {
		...
		assert_eq!(body["format_version"], weft_physical_type::PAGED_SEGMENT_FORMAT_VERSION);
	...
	async fn ingest_value_exceeding_tolerance_is_rejected() {
		...
		store.declare("counts", &weft_physical_type::AspectSchema::new(weft_physical_type::PhysicalType::ScaledI64 { scale: 0 }, "0".parse().unwrap(), weft_physical_type::TimeUnit::Seconds)).await.unwrap();
	...
	async fn ilp_ingest_require_sorted_passes_because_the_parser_pre_sorts() {
		// The `weft-line-protocol` parser returns points sorted by timestamp, so an
	...
	async fn router_with_declared_temp(dir: &TempDir) -> axum::Router {
		let store = Arc::new(SegmentStore::open(dir.path()).await.expect("opens store"));
		store.declare("temp", &weft_physical_type::AspectSchema::new(weft_physical_type::PhysicalType::F64, "0".parse().unwrap(), weft_physical_type::TimeUnit::Seconds)).await.expect("declares");
		app_with_state(AppState::new().with_store(store))
	}
	...
	async fn ingest_increments_shared_ingest_metrics() {
		...
		let store = Arc::new(SegmentStore::open(dir.path()).await.unwrap());
		store.declare("price", &weft_physical_type::AspectSchema::new(weft_physical_type::PhysicalType::F64, "0".parse().unwrap(), weft_physical_type::TimeUnit::Seconds)).await.unwrap();
	...
	fn epoch_in_unit_rescales_to_the_declared_unit() {
		...
		assert_eq!(super::epoch_in_unit(instant, weft_physical_type::TimeUnit::Seconds), Some(100));
		assert_eq!(super::epoch_in_unit(instant, weft_physical_type::TimeUnit::Millis), Some(100_000));
		assert_eq!(super::epoch_in_unit(instant, weft_physical_type::TimeUnit::Micros), Some(100_000_000));
		assert_eq!(super::epoch_in_unit(instant, weft_physical_type::TimeUnit::Nanos), Some(100_000_000_000));
	}
}
