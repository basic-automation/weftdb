...
impl PolynomialSpline {
    ...
    fn fit_linear(p1: &Measurement, p2: &Measurement) -> Result<PolynomialSegment> {
        let dt = BigDecimal::from_i64((p2.timestamp - p1.timestamp).num_milliseconds())
    ...
    fn fit_polynomial(points: &[Measurement], degree: usize) -> Result<PolynomialSegment> {
        ...
        for point in points {
            let t = BigDecimal::from_i64((point.timestamp - base_time).num_milliseconds())
                .context("Failed to convert timestamp to BigDecimal")?;
            times.push(t);
...
mod tests {
    ...
    fn test_polynomial_degree_3() {
        ...
        let measurements = vec![
            ...
            create_measurement(dataset_id, Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 20).unwrap(), 80.0),
            create_measurement(dataset_id, Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 30).unwrap(), 270.0)
        ];
    ...
    fn test_polynomial_extrapolation_backward() {
        ...
        let measurements = vec![
            create_measurement(dataset_id, Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 10).unwrap(), 100.0), 
            create_measurement(dataset_id, Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 20).unwrap(), 200.0), 
            create_measurement(dataset_id, Utc.with_ymd_and_hms(2023, 1, 1, 0, 0, 30).unwrap(), 300.0)
        ];
