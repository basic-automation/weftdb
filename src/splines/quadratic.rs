...
impl QuadraticSpline {
        ...
        if n == 2 {
            ...
            for i in 0..n - 1 {
                if i + 2 < n {
                    ...
                    let dt = BigDecimal::from_i64((measurements[i + 1].timestamp - measurements[i].timestamp).num_milliseconds())
                        .context(format!("Failed to convert timestamp difference to BigDecimal for final segment {i}"))?;
    ...
    fn fit_parabola(p1: &Measurement, p2: &Measurement, p3: &Measurement) -> Result<QuadraticSegment> {
        ...
        let t2 = BigDecimal::from_i64((p2.timestamp - p1.timestamp).num_milliseconds())
            .context("Failed to convert timestamp difference to BigDecimal for p2")?;
        let t3 = BigDecimal::from_i64((p3.timestamp - p1.timestamp).num_milliseconds())
            .context("Failed to convert timestamp difference to BigDecimal for p3")?;
