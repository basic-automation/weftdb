//! # weft-line-protocol
//!
//! `InfluxDB` Line Protocol (ILP) parsing for WeftDB ingest.
//!
...
//!
//! This crate is deliberately a *format* parser, not a vendor connector: it turns
//! ILP text into neutral [`LineRecord`]s (and, via [`parse_points`], into
//! `splimes::Point`s). It pulls in no `InfluxDB` client, no network, and no
//! vendor-specific dependency, so it honors the connector hard-constraint — a
//! concrete `InfluxDB` *connector* (talking to a running server) would live in its
//! own crate outside the core. It is shared by the benchmark harness (`weft-bench`)
//! and the HTTP server (`weft-server`) so both speak exactly the same dialect.
//!
...
/// chosen numeric field — the primary on-ramp for TSBS-style data.
///
