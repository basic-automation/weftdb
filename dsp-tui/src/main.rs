...
async fn main() {
	...
	let log_path = std::env::var("DSP_DATA_DIR").map_or_else(
		|_| {
			let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
			home.join(".dsp").join("dsp-tui.log")
		},
		|data_dir| std::path::PathBuf::from(data_dir).join("dsp-tui.log"),
	);
	...
	tracing_subscriber::registry().with(fmt::layer().with_writer(std::sync::Arc::new(file)).with_ansi(false).with_target(true).with_level(true).with_thread_ids(true)).with(LogBufferLayer::new(log_buffer.clone())).with(EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("dsp_tui=debug,database=debug,info"))).init();

	eprintln!("📝 Logs written to: {}", log_path.display());
	tracing::info!("=== Starting DSP TUI application ===");
