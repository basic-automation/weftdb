...
fn draw_mode_selection(f: &mut Frame, app: &App, area: Rect) {
	...
	let list = List::new(items).block(Block::default().borders(Borders::ALL).title("DSP Database Manager - Select Mode")).highlight_style(Style::default().fg(Color::Yellow)).highlight_symbol(">> ");
