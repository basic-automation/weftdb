...
static UNPROCESSED_BATCHES_QUEUE: LazyLock<Mutex<HashMap<AspectId, Vec<Batch>>>> = LazyLock::new(|| Mutex::new(HashMap::new()));

#[allow(dead_code)]
static PROCESSED_BATCHES_QUEUE: LazyLock<Mutex<HashMap<AspectId, Vec<Batch>>>> = LazyLock::new(|| Mutex::new(HashMap::new()));
...
pub async fn build_unprocessed_queue(database: &Database, aspect: &AspectId, resolution: &Resolution, method: &Spline, batch_size: usize) -> Result<()> {
	...
	let point_stream = database.stream_analyze_range(*aspect, start_time, end_time, *resolution, *method);
	tokio::pin!(point_stream);

	let mut points = Vec::new();
	while let Some(point) = point_stream.next().await {
		// add batch to batches up to BATCH_SIZE then process batch and clear
		let point = point?;
		points.push(point);
		if points.len() >= batch_size {
			let measurements = points.iter().map(|p| BatchedMeasurement::new(p.clone())).collect();
			let batch = Batch::new(points.len(), measurements, *resolution);

			let mut batches_lock = UNPROCESSED_BATCHES_QUEUE.lock().await;
			let entry = batches_lock.entry(*aspect).or_insert_with(Vec::new);
			entry.push(batch);
	...
	if !points.is_empty() && points.len() >= batch_size {
		let measurements = points.iter().map(|p| BatchedMeasurement::new(p.clone())).collect();
		let batch = Batch::new(points.len(), measurements, *resolution);
...
mod tests {
	...
	async fn test_build_unprocessed_queue() -> Result<()> {
		...
		let aspect = aspects.iter().find(|a| a.name() == "close").ok_or_else(|| anyhow::anyhow!("Aspect 'close' not found"))?;
	...
	fn generate_specific_test_batch() -> Batch {
		...
		Batch::new(measurements.len(), measurements, Resolution::Seconds)
	}
