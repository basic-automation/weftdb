//! # weft-arrow
//!
//! Apache Arrow interchange for WeftDB's typed columnar segments (roadmap
...
//!
//! Arrow is the lingua franca a WeftDB segment must speak to reach the wider
...
//! hot-path crates (`splimes`, `database`, `weft-physical-type`) lean. So the
//! conversion lives **here**, in a leaf crate that depends on
//! [`weft_physical_type`] (never the reverse): the heavy dependency never reaches
//! the core.
//!
...
//! (constraint #3 — WeftDB's own `.weftseg` columnar segments still own the hot path),
...
pub const META_TIME_UNIT: &str = "weft:time_unit";
...
/// [`META_VALUE_ENCODING`]).
pub const META_PHYSICAL_TYPE: &str = "weft:physical_type";
/// Schema-metadata key carrying the wire form of the value column.
pub const META_VALUE_ENCODING: &str = "weft:value_encoding";
/// Schema-metadata key carrying the segment's format version.
pub const META_FORMAT_VERSION: &str = "weft:format_version";

/// The pre-rename schema-metadata prefix, used when the project was called DSP (`dsp:time_unit`
/// and friends).
///
/// Writers only ever emit the current `weft:` keys, but [`legacy_metadata_key`] lets readers
/// accept a batch exported before the rename, so Arrow/Parquet artifacts written by an older
/// build still import.
const LEGACY_META_PREFIX: &str = "dsp:";

/// The pre-rename spelling of a `weft:`-prefixed metadata `key`, for the backward-compatible
/// read path. Returns `None` for a key that does not carry the current prefix.
fn legacy_metadata_key(key: &str) -> Option<String> {
	key.strip_prefix("weft:").map(|rest| format!("{LEGACY_META_PREFIX}{rest}"))
}

/// Look a schema-metadata `key` up on `batch`, falling back to its pre-rename `dsp:` spelling
/// so batches exported before the rename still read.
fn metadata_lookup<'a>(batch: &'a RecordBatch, key: &str) -> Option<&'a String> {
	let meta = batch.schema_ref().metadata();
	meta.get(key).or_else(|| legacy_metadata_key(key).and_then(|legacy| meta.get(&legacy)))
}
...
/// [`ScaledI64`](weft_physical_type::PhysicalType::ScaledI64) /
/// [`ScaledI128`](weft_physical_type::PhysicalType::ScaledI128) encodings.
...
/// ([`columns_to_record_batch`]) can span many `.weftseg` segments of differing
/// format versions and physical encodings, so no single
/// [`SEGMENT_FORMAT_VERSION`](weft_physical_type::Segment::version) applies. `0`
...
/// Why a [`RecordBatch`] could not be converted back into WeftDB columns.
...
/// Build a self-describing, lossless Arrow [`RecordBatch`] directly from WeftDB's
...
/// Build a self-describing Arrow [`RecordBatch`] from WeftDB's logical columns using
...
/// ([`AspectSchema::value`](weft_physical_type::AspectSchema)), so the whole logical
...
/// Read an exported [`RecordBatch`] back into WeftDB's logical columns.
///
...
fn unit_from_metadata(batch: &RecordBatch) -> Result<TimeUnit, ConvertError> {
	let unit_name = metadata_lookup(batch, META_TIME_UNIT).ok_or(ConvertError::MissingMetadata(META_TIME_UNIT))?;
...
/// Concatenate a stream of exported [`RecordBatch`]es back into one pair of WeftDB
...
/// metadata). The self-describing schema (including WeftDB's [`META_TIME_UNIT`] and
/// physical-encoding metadata) is written once at the head of the stream, so
/// [`read_ipc_stream`] reconstructs the batches — and WeftDB's columns from them —
...
/// [`segment_from_record_batch`]) to recover WeftDB's columns.
///
/// # Errors
///
...
/// self-describing Arrow schema — including WeftDB's [`META_TIME_UNIT`] and
/// physical-encoding metadata — is embedded in the Parquet file (Parquet preserves
/// Arrow schema metadata), so [`read_parquet`] reconstructs the batches, and WeftDB's
...
pub fn read_parquet(bytes: &[u8]) -> Result<Vec<RecordBatch>, ConvertError> {
	...
	// The builder restores the embedded Arrow schema (with WeftDB's schema metadata);
...
/// interpolation series** — the output of WeftDB's flagship interpolate-on-read path,
...
///   documented wire-numeric boundary is `f64` (WeftDB's logical `BigDecimal` is
...
///
/// The one-call bridge the `weft-server` interpolate endpoint uses: it holds only
...
/// output of WeftDB's downsample/aggregation path.
///
...
///
/// The one-call bridge the `weft-server` downsample endpoint uses, mirroring
...
mod tests {
	...
	use weft_physical_type::AspectSchema;
