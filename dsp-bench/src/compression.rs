...
//!
//! The workload seals the corpus into a `.dspseg` segment, then times a full decode
...
//!
//! Like the other storage workloads it drives DSP's own `dsp-physical-type` path
//! directly (no `database`/libSQL), keeping DSP-Bench a vendor-neutral leaf crate.

use std::time::Instant;

use bigdecimal::{BigDecimal, ToPrimitive};
use dsp_physical_type::{dspseg::read_segment, Segment, TimeUnit};
...
///
/// Each shape stresses a different codec regime, so the workload surfaces how DSP's
...
/// the whole segment from its `.dspseg` bytes (`read_segment` + `decode_nullable`) and
