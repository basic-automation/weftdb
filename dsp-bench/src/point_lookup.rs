...
//!
//! The whole 2026-07-13 read-path arc built DSP's streaming point read: a
...
//! (`dspseg::read_segment_point`), and amortizes the timestamp decode across many
...
//! regular-vs-irregular). This module lifts that into a *customer-runnable* DSP-Bench
...
//! evidence for the *When DSP beats general TSDBs* point-lookup positioning.
//!
...
//!
//! The storage-backed path drives DSP's own hot path directly: it seals a
//! `dsp-physical-type` [`Segment`] from the profile dataset and times
//! `read_segment_point` / `read_segment_points` over the `.dspseg` bytes — no
//! `database`/libSQL control plane is pulled into the harness (keeping DSP-Bench a
...
/// `.dspseg` segment and resolves `query_count` instants against it.
...
/// rep re-resolves the whole query batch against the sealed `.dspseg` bytes and the
