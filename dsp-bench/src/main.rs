...
fn parse_aggregations(s: &str) -> Result<Vec<Aggregation>, String> {
	let aggs: Vec<Aggregation> = s.split(',').map(str::trim).filter(|t| !t.is_empty()).map(|t| Aggregation::from_token(t).ok_or_else(|| format!("invalid --ds-aggs token `{t}` (use min/max/avg/sum/first/last/p50/p90/p95/p99/twa/twa_linear)"))).collect::<Result<_, _>>()?;
	if aggs.is_empty() {
