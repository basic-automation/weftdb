...
//! sophistication behind it. It is the honest reference DSP's native path is
//! measured against — until now DSP-Bench had only one system ([`DspAdapter`]) and
...
//!   *baseline*. Comparing DSP's chosen method against it is a quality/speed
...
//!   the same hard constraint as the rest of DSP. Values therefore stay finite and
//!   pass the harness correctness gate exactly as the DSP path does.
...
/// interpolation — the portable client-side baseline DSP is compared against.
