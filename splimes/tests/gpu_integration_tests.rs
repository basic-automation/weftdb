...
fn test_gpu_config_presets_are_applied_or_refused_with_a_reason() {
	for (name, config) in [("default", GpuConfig::default()), ("low_memory", GpuConfig::low_memory()), ("high_performance", GpuConfig::high_performance()), ("minimal", GpuConfig::minimal())] {
		match prewarm_gpu_with_config(config.clone()) {
			Ok(()) => {
				...
				assert_eq!(effective.buffer_pool.max_pool_memory, config.buffer_pool.max_pool_memory, "{name}: the effective pool size must be the one requested");
				assert_eq!(effective.num_staging_buffers, config.num_staging_buffers, "{name}: the effective staging count must be the one requested");
			}
			Err(e) => {
				let msg = e.to_string();
				assert!(msg.contains("already initialized") || msg.contains("already requested") || msg.contains("GPU") || msg.contains("adapter"), "{name}: a refusal must say why; got: {msg}");
			}
		}
	}
}
...
fn test_buffer_pool_stats_available() {
	...
	assert!(stats.total_allocated_bytes < 1024 * 1024 * 1024, "Total allocated should be reasonable (< 1GB)");
}
...
fn test_config_presets_are_different() {
	...
	assert_ne!(default.buffer_pool.max_pool_memory, low_mem.buffer_pool.max_pool_memory, "Default and low_memory should have different pool sizes");
	assert_ne!(default.buffer_pool.max_pool_memory, high_perf.buffer_pool.max_pool_memory, "Default and high_performance should have different pool sizes");
	assert_ne!(low_mem.buffer_pool.max_pool_memory, minimal.buffer_pool.max_pool_memory, "Low_memory and minimal should have different pool sizes");

	// Verify staging buffer counts are different
	assert_ne!(default.num_staging_buffers, low_mem.num_staging_buffers, "Default and low_memory should have different staging buffer counts");
}
...
fn test_config_presets_ordered_by_resources() {
	...
	assert!(minimal.buffer_pool.max_pool_memory < low_mem.buffer_pool.max_pool_memory, "Pool memory should increase from minimal to low_memory");
	assert!(low_mem.buffer_pool.max_pool_memory < default.buffer_pool.max_pool_memory, "Pool memory should increase from low_memory to default");
	assert!(default.buffer_pool.max_pool_memory < high_perf.buffer_pool.max_pool_memory, "Pool memory should increase from default to high_performance");

	// Verify staging buffer counts increase monotonically
	assert!(minimal.num_staging_buffers <= low_mem.num_staging_buffers, "Staging buffers should increase from minimal to low_memory");
	assert!(low_mem.num_staging_buffers <= default.num_staging_buffers, "Staging buffers should increase from low_memory to default");
	assert!(default.num_staging_buffers <= high_perf.num_staging_buffers, "Staging buffers should increase from default to high_performance");
}
