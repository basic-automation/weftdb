...
use splimes::{GpuConfig, prewarm_gpu_with_config};
...
fn bench_gpu_config_impact(c: &mut Criterion) {
	...
	let configs = vec![("minimal", GpuConfig::minimal()), ("low_memory", GpuConfig::low_memory()), ("default", GpuConfig::default()), ("high_performance", GpuConfig::high_performance())];
...
fn bench_pool_memory_configs() {
	...
	for (name, config) in configs {
		...
		println!("{:20} | Pool: {:6.1}MB | Staging Buffers: {} | Command Batch: {}", name, pool_mb, config.num_staging_buffers, config.max_command_batch_size);
	}
}
