...
use splimes::{Point, Resolution, Spline, cpu_interpolate, gpu_interpolate, parallel_interpolate, prewarm_gpu};
...
fn analyze_should_use_gpu_thresholds(c: &mut Criterion) {
	...
	let test_sizes = vec![(100, "100_boundary"), (1_000, "1k_medium"), (10_000, "10k_medium"), (100_000, "100k_boundary")];
	...
	for (input_count, label) in test_sizes {
		...
		group.bench_with_input(BenchmarkId::new("CPU", label), &input_count, |b, &_count| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter(|| {
				rt.block_on(async {
					let _ = cpu_interpolate(&mut points.clone(), start, end, resolution, spline).await;
				});
			});
		});

		group.bench_with_input(BenchmarkId::new("Parallel", label), &input_count, |b, &_count| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter(|| {
				rt.block_on(async {
					let _ = parallel_interpolate(&mut points.clone(), &start, &end, spline, resolution).await;
				});
			});
		});

		group.bench_with_input(BenchmarkId::new("GPU", label), &input_count, |b, &_count| {
			let rt = tokio::runtime::Runtime::new().unwrap();
			b.iter(|| {
				rt.block_on(async {
					let _ = gpu_interpolate(&mut points.clone(), start, end, resolution, spline).await;
				});
			});
		});
	}
