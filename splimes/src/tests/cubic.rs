...
mod tests {
	...
	use crate::{
		Resolution, auto_interpolate, gpu::types::GpuInterpolator, gpu_interpolate, helpers::TargetTimesIterator, parallel_interpolate, splines::cubic, tests::{
	...
	async fn test_cubic_interpolation() {
		...
		println!("Cubic: Input count: {input_count}, Output count: {output_count}");
		...
		if auto_time < gpu_time && auto_time < cpu_time && auto_time < parallel_time {
			...
			println!("Cubic: Auto was faster by {percentage_difference:.2}%");
			...
			println!("Cubic: GPU was faster by {percentage_difference:.2}%");
			...
			println!("Cubic: CPU was faster by {percentage_difference:.2}%");
			...
			println!("Cubic: SIMD was faster by {percentage_difference:.2}%");
		} else {
			...
			println!("Cubic: Parallel SIMD was faster by {percentage_difference:.2}%");
		}
	}
