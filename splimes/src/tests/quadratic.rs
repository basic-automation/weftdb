...
mod tests {
	...
	use crate::{
		Resolution, auto_interpolate, gpu::types::GpuInterpolator, gpu_interpolate, helpers::TargetTimesIterator, parallel_interpolate, splines::quadratic, tests::{
	...
	async fn test_quadratic_interpolation() {
		...
		let spline = crate::Spline::Quadratic; // Using quadratic spline for this test
		let input_count = points.len();
		let output_count = target_times_iter.estimate_len().unwrap();
		println!("Quadratic: Input count: {input_count}, Output count: {output_count}");

		// cpu interpolation
		println!("Quadratic: CPU Test Starting...");
		let timer = tokio::time::Instant::now();
		let cpu = quadratic(&mut points, &start, &end, &RESOLUTION).await.unwrap();
		...
		plot_terminal("Quadratic: Parallel Interpolation Results", parallel.clone()).unwrap();
		drop(parallel);

		// compare cpu and parallel results
		println!("Quadratic: Comparing CPU and Parallel results...");
		...
		// gpu interpolation
		println!("Quadratic: GPU Test Starting...");
		let timer = tokio::time::Instant::now();
		...
		println!("Quadratic: Comparing CPU and GPU results...");
		...
		println!("Quadratic: Auto Test Starting...");
		...
		println!("Quadratic: Comparing CPU and Auto results...");
		...
		if auto_time < gpu_time && auto_time < cpu_time && auto_time < parallel_time {
			...
			println!("Quadratic: Auto was faster by {percentage_difference:.2}%");
			...
			println!("Quadratic: GPU was faster by {percentage_difference:.2}%");
			...
			println!("Quadratic: CPU was faster by {percentage_difference:.2}%");
			...
			println!("Quadratic: SIMD was faster by {percentage_difference:.2}%");
		} else {
			...
			println!("Quadratic: Parallel SIMD was faster by {percentage_difference:.2}%");
		}
	}
