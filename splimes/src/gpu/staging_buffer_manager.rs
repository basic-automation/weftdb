...
impl std::fmt::Debug for MappedStagingBuffer {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("MappedStagingBuffer").field("state", &self.state).field("has_submission_index", &self.submission_index.is_some()).finish_non_exhaustive()
	}
}
...
impl StagingBufferManager {
	...
	pub const fn new(buffer_size: u64, max_buffers: usize) -> Self {
		Self { buffers: Mutex::new(Vec::new()), submission_order: Mutex::new(VecDeque::new()), current_index: AtomicUsize::new(0), buffer_size, max_buffers }
	}
	...
	pub fn acquire(&self, device: &Device, _queue: &Queue) -> Result<AcquiredStaging> {
		...
		if buffers.len() < self.max_buffers {
			let buffer = Arc::new(device.create_buffer(&wgpu::BufferDescriptor { label: Some("Streaming Staging Buffer"), size: self.buffer_size, usage: BufferUsages::COPY_DST | BufferUsages::MAP_READ, mapped_at_creation: false }));

			let index = buffers.len();
			buffers.push(MappedStagingBuffer { buffer: buffer.clone(), state: BufferState::Free, submission_index: None });

			return Ok(AcquiredStaging { buffer, buffer_index: index });
		}

		// Find a free buffer
		for (idx, staged) in buffers.iter_mut().enumerate() {
			if staged.state == BufferState::Free {
				return Ok(AcquiredStaging { buffer: staged.buffer.clone(), buffer_index: idx });
			}
		}
		...
		if let Some(oldest_idx) = oldest_idx {
			...
			if let Some(ref sub_idx) = buffers[oldest_idx].submission_index {
				let _ = device.poll(wgpu::PollType::Wait { submission_index: Some(sub_idx.clone()), timeout: None });
			}
		...
		Ok(AcquiredStaging { buffer: buffers[idx].buffer.clone(), buffer_index: idx })
	}
