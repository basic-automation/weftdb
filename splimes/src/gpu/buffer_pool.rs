...
impl BufferPool {
	...
	pub fn acquire(&self, size: u64, buffer_type: PooledBufferType, label: Option<&str>) -> Result<(Arc<Buffer>, bool)> {
		if size == 0 {
	...
	#[allow(clippy::significant_drop_tightening)] // Lock held intentionally during entire pool operation
	pub fn acquire_with_metadata(&self, size: u64, buffer_type: PooledBufferType, label: Option<&str>) -> Result<AcquiredBuffer> {
