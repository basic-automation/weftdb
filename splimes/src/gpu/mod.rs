...
pub async fn gpu_interpolate(points: &mut [Point], start: DateTime<Utc>, end: DateTime<Utc>, resolution: Resolution, spline: Spline) -> Result<Vec<Point>> {
	...
	if GpuInterpolator::supports_f64_static()? { gpu_interpolate_f64(points, &start, &end, resolution, &method, &spline) } else { batch(points, &start, &end, &spline, &resolution, |state| Box::pin(gpu_interpolate_f32(state))).await }
}
